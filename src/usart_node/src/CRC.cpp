...
const std::uint8_t CRC8_INIT = 0xff;
const std::uint16_t CRC_INIT = 0xffff;
const std::uint8_t CRC8_TAB[256] = {
...
const std::uint16_t wCRC_Table[256] = {
...
void Append_CRC8_Check_Sum(std::uint8_t *pchMessage, std::uint32_t dwLength)
{
    ...
    if ((pchMessage == 0) || (dwLength <= 2))
        return;
    ucCRC = Get_CRC8_Check_Sum((std::uint8_t *)pchMessage, dwLength - 1, CRC8_INIT);
...
std::uint16_t Get_CRC16_Check_Sum(std::uint8_t *pchMessage, std::uint32_t dwLength, std::uint16_t wCRC)
{
    ...
    {
        chData = *pchMessage++;
        (wCRC) = ((std::uint16_t)(wCRC) >> 8) ^ wCRC_Table[((std::uint16_t)(wCRC) ^ (std::uint16_t)(chData)) & 0x00ff];
    }
    return wCRC;
}
...
void Append_CRC16_Check_Sum(std::uint8_t *pchMessage, std::uint32_t dwLength)
{
    ...
    if ((pchMessage == NULL) || (dwLength <= 2))
        return;
    wCRC = Get_CRC16_Check_Sum((std::uint8_t *)pchMessage, dwLength - 2, CRC_INIT);
    pchMessage[dwLength - 2] = (std::uint8_t)(wCRC & 0x00ff);
    pchMessage[dwLength - 1] = (std::uint8_t)((wCRC >> 8) & 0x00ff);
}
